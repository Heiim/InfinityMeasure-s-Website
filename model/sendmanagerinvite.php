...
if ($stmt = $con->prepare('SELECT idaccount, password FROM accounts WHERE email = ?')) {
    ...
    if ($stmt->num_rows > 0) {
        ...
        if ($stmt = $con->prepare('INSERT INTO managertokens (email, token) VALUES (?,?)')) {
            ...
            $subject = 'Création d\'un compte gestionnaire';
            //$headers = 'From: ' . $from . "\r\n" . 'X-Mailer: PHP/' . phpversion() . "\r\n" . 'MIME-Version: 1.0' . "\r\n" . 'Content-Type: text/html; charset=UTF-8' . "\r\n";
            $register_link = 'https://infinite-measures-quirky.000webhostapp.com/index.php?action=managerregister&email=' . $_POST['email'] . '&token=' . $token . '&company_code=' . $_POST['company_code'];
