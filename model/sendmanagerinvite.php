...
if ($stmt = $con->prepare('SELECT idaccount, password FROM accounts WHERE email = ?')) {
    ...
    if ($stmt->num_rows > 0) {
        ...
        if ($stmt = $con->prepare('INSERT INTO managertokens (email, token) VALUES (?,?)')) {
            ...
            $register_link = 'https://infinite-measures-g7e.000webhostapp.com/index.php?action=managerregister&email=' . $_POST['email'] . '&token=' . $token . '&company_code=' . $_POST['company_code'];
